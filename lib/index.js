...
if (!fs.existsSync(path)) fs.writeJsonSync(path, { welcome: {}, goodbye: {}, sudos: [], languages: {} });
...
module.exports = {
    ...
    /**
     * Sets the language for a group
     */
    setLanguage: (chatId, lang) => {
        const db = fs.readJsonSync(path);
        if (!db.languages) db.languages = {};
        db.languages[chatId] = lang;
        fs.writeJsonSync(path, db, { spaces: 2 });
    },
    
    /**
     * Gets the language set for a group (defaults to 'en')
     */
    getLanguage: (chatId) => {
        const db = fs.readJsonSync(path);
        return (db.languages && db.languages[chatId]) || 'en';
    },
